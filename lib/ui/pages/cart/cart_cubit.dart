...
class CartCubit extends Cubit<CartState> {
  ...
  Future<void> getAllCart(UserEntity userEntity) async {
    emit(state.copyWith(addToCartStatus: LoadStatus.loading));
    try {
      final cartList = await cartRepository.getAllCart(userEntity);
      int totalAllPrice = 0;
  ...
  Future<void> deleteAllCart(UserEntity userEntity) async {
    emit(state.copyWith(deleteAllCartStatus: LoadStatus.loading));
    try {
      await cartRepository.deleteAllCart(userEntity);
      emit(
        state.copyWith(
            deleteAllCartStatus: LoadStatus.success,
            cartList: [],
            totalAllPrice: 0),
      );
    } catch (e) {
      emit(
        state.copyWith(deleteAllCartStatus: LoadStatus.failure),
      );
    }
  }
}
